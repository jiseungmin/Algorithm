...
def solution(food_times, k):
    ...
    # 우선 순위 큐의 남아있는 요소들을 음식 번호를 기준으로 오름차순 정렬
    result = sorted(q, key=lambda x: x[1])

    # (K 발생까지 남은 시간 % 남은 음식 개수), 배열의 idx는 0부터 시작하므로 나머지 값을 idx로 사용하면됨
